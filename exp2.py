...
df_account = spark.read \
                    ...
                    .load(r"E:\Machine Learning Engineer\Pyspark\Spark\SBDL - Starter\test_data\accounts\account_samples.csv")

df_parties = spark.read \
                    .format("csv")\
                    .option('header', 'true')\
                    .option("infer_schema", "true")\
                    .load(r"E:\Machine Learning Engineer\Pyspark\Spark\SBDL - Starter\test_data\parties\party_samples.csv")

df_address = spark.read \
                    .format("csv")\
                    .option('header', 'true')\
                    .option("infer_schema", "true")\
                    .load(r"E:\Machine Learning Engineer\Pyspark\Spark\SBDL - Starter\test_data\party_address\address_samples.csv")

#df_account.show()
#df_parties.show()
#df_address.show()

# get_contract(df_account).show()
# get_contract(df_account).printSchema()

# get_relations(df_parties).show()
# get_relations(df_parties).printSchema()

# get_address(df_address).show()
# get_address(df_address).printSchema()

# join_party_address(get_relations(df_parties), get_address(df_address)).show()
# join_party_address(get_relations(df_parties), get_address(df_address)).printSchema()

df_party_address = join_party_address(get_relations(df_parties), get_address(df_address))
df_party_address_contract = join_contract_party(get_contract(df_account), df_party_address)
df_party_address_contract.show()
apply_header(spark, df_party_address_contract).show()
apply_header(spark, df_party_address_contract).select(col("payload.contractIdentifier.newValue").alias("key"),\
                                                struct("*").alias("value")).printSchema()
# get_relations(df_parties).join(get_address(df_address),"party_id", "left_outer").show()